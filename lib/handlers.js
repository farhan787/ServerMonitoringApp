      ...
      if (err) {
        ...
        if (hashedPassword) {
          ...
          _data.create('users', phone, userObject, (err) => {
            if (!err) callback(200, { Status: 'User created successfully!!' });
            else {
              console.log(err);
      ...
      if (tokenIsValid) {
        _data.read('users', phone, (err, data) => {
          if (!err && data) {
            ...
            callback(200, data);
          } else callback(404, { Error: "User doesn't exist" });
        });
      } else
        callback(400, {
    ...
    handlers.verifyToken(token, phone, (tokenIsValid) => {
      if (tokenIsValid) {
        ...
        _data.read('users', phone, (err, userData) => {
          ...
          } else callback(404, { Error: "The specified user doesn't exist" });
        });
      } else
        callback(400, {
          Error: 'Missing required token in header or token is invalid',
          ...
          if (!err && userData) {
            _data.delete('users', phone, (err) => {
              if (!err) {
                    ...
                    _data.delete('checks', checkId, (err) => {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors)
                          callback(200, {
                            Status:
                              'User and all the information associated with this user has been deleted successfully!!',
                          });
                        else
                          callback(500, {
                            Error:
                              "Error is encountered while attempting to delete all of the user's checks. All checks may not have been deleted from the system successfully.",
                          });
                      }
                    });
                  });
                ...
                callback(500, {
                  Error: 'Could not delete the user, it may already be deleted',
                });
            });
          } else
            callback(404, {
              Error:
                'Could not find the specified user, check the phone number again or the token does not belong to this phone number',
            });
        });
      } else
        callback(400, {
  ...
  if (phone && password) {
    ...
    _data.read('users', phone, (err, userData) => {
      if (!err && userData) {
        ...
        } else callback(401, { Error: 'Incorrect password' });
  ...
  if (id) {
    _data.read('tokens', id, (err, tokenData) => {
      if (!err && tokenData) {
        ...
        callback(404, {
          Error: 'Could not find the token for the specified token id',
        });
    });
  } else callback(400, { Error: 'Missing token id inside query string' });
};
...
handlers._tokens.put = function (data, callback) {
  ...
  if (id && extend) {
    ...
    _data.read('tokens', id, (err, tokenData) => {
      if (!err && tokenData) {
        ...
        if (tokenData.expires > Date.now()) {
          ...
          _data.update('tokens', id, tokenData, (err) => {
            if (!err)
              callback(200, {
                Status: 'Token expiration extended successfully',
              });
            else
      ...
      } else callback(404, { Error: "Token doesn't exist" });
    });
  } else
  ...
  if (id) {
    ...
    _data.read('tokens', id, (err, tokenData) => {
      if (!err && tokenData) {
        ...
        callback(400, {
          Error: 'Invalid token id or token may be already deleted',
        });
    });
  } else callback(400, { Error: 'Missing token id in the query string' });
};
...
handlers._checks.post = function (data, callback) {
  ...
  if (protocol && url && method && successCodes && timeoutSeconds) {
    ...
    _data.read('tokens', token, (err, tokenData) => {
      ...
      } else callback(401, { Error: 'Missing token in the headers' });
    });
  } else
...
handlers._checks.get = function (data, callback) {
  ...
  } else callback(400, { Error: 'Invalid check id in the query string' });
};
            ...
            if (isTokenValid) {
              ...
              _data.update('checks', id, checkData, (err) => {
                if (!err) {
                  ...
                  callback(500, {
                    Error:
                      'Could not update the check data of the specified check id',
                  });
              });
            } else
...
handlers._checks.delete = function (data, callback) {
              ...
              _data.read('users', checkData.userPhone, (err, userData) => {
                if (!err && userData) {
                  ...
                  if (checkIndex > -1 && checkIndex < config.maxChecks) {
                    ...
                    _data.update(
                      ...
                      (err) => {
                        if (!err) {
                          ...
                          callback(500, {
                            Error:
                              'Could not update the user, so could not delete the check',
                          });
                      }
                    );
                  } else
                    callback(500, {
                      Error:
                        "Could not find the position of check inside user's object",
                    });
                } else
                  callback(500, {
                    Error:
  ...
  } else callback(400, { Error: 'Missing valid check id in the query string' });
};
