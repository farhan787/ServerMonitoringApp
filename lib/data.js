          ...
          fs.writeFile(fileDescriptor, stringData, (err) => {
            if (!err) {
              fs.close(fileDescriptor, (err) => {
                ...
                else callback('Error closing the file while updating');
              });
            } else callback('Error writing to the file while updating');
          });
...
lib.delete = function (dir, file, callback) {
  ...
  fs.unlink(filePath, (err) => {
    if (!err) callback(false);
    else callback("Can't delete this file or it may be already deleted");
  });
};
